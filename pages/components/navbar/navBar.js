Component({
  ...
  attached: function() {
    // 路由长度为1时，返回按钮默认为false
    let back = this.data.back;
    if(back && getCurrentPages().length === 1){
      this.setData({
        back:false
      })
    }
  ...
  methods: {
    ...
    getSystemInfo() {
      ...
      if (app.globalSystemInfo && !app.globalSystemInfo.ios) {
        ...
        let rect = (wx.getMenuButtonBoundingClientRect && wx.getMenuButtonBoundingClientRect()) || { bottom: 58, height: 32, left: 317, right: 404, top: 26, width: 87};
